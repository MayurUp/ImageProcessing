...
function ak_img_watermark($target, $wtrmrk_file, $newcopy) { 
    //$target = convertImageToGrayscale($target, .75);
    ...
    $dst_x = ($img_w * 0.03); // For centering the watermark on any image
    $dst_y = ($img_h) - ($img_h*0.97); // For centering the watermark on any image
...
function ak_img_watermarksecond($target, $wtrmrk_file, $newcopy) { 
    //$target = convertImageToGrayscale($target, .75);
    ...
    $wtrmrk_w = imagesx($watermark); 
    $wtrmrk_h = imagesy($watermark); 
    $dst_x = ($img_w/2) - ($wtrmrk_w/4); // For centering the watermark on any image
    $dst_y = ($img_h*.75) - ($wtrmrk_h/2); // For centering the watermark on any image
    imagecopy($img, $watermark, $dst_x, $dst_y, 0, 0, $wtrmrk_w, $wtrmrk_h); 
    imagejpeg($img, $newcopy, 100); 
    imagedestroy($img); 
    imagedestroy($watermark); 
